...
var i = g();
i.next();
i.throw(new Error('an error occurred!'));
...
```javascript
gen.throw(new Error('an error occurred')); // Uncaught Error: an error occurred
// Equivalent to replacing let result = yield x + y
// with let result = throw(new Error('an error occurred'));
```
