---
title: Axios Instance Module Unit Tests
...
`ts-axios` provides the `axios.create` static method, which returns an `instance`. We need to test this module.
...
With that, we have completed the tests for the business logic related to the `Axios` instance module of the `ts-axios` library. In the next lesson, we will test the interceptors module.
