<template>
  <div class="custom-page archives-page">
    <div class="theme-vdone-wrapper">
      ...
      <div class="count">
        Total <i>{{ $sortPostsByDate.length }}</i> articles
      </div>
      <ul>
...
export default {
  ...
  data() {
    return {
      postsList: [],
      countByYear: {}, // Article count by year

      perPage: 80, // Items per page
  ...
  created() {
    ...
    // Calculate article count by year
...
.theme-style-line
  .archives-page .theme-vdone-wrapper
    box-shadow 0 0
    // border 1px solid var(--borderColor)
    // border-radius 5px
.archives-page
  .theme-vdone-wrapper
    @extend $vdone-wrapper
    position relative
